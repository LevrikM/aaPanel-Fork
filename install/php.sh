...
Env_check(){
	...
	UBUNTU_VER=$(cat /etc/issue|grep -i ubuntu|awk '{print $2}'|cut -f 1 -d '.')
	if [ "${DEBIAN_VER}" == "13" ] || [ "${UBUNTU_VER}" == "26" ];then
...
if [ "$2" == "8.4" ] || [ "$2" == "8.5" ];then
	xml_minor_version=$(xml2-config --version|cut -d '.' -f 2)
	xml_patch_version=$(xml2-config --version|cut -d '.' -f 3)
	if [ "${xml_minor_version}" ];then
		if [ "${xml_minor_version}" -le 9 ] && [ "${xml_patch_version}" -lt 4 ];then
			if [ "${AA_PANEL_CHECK}" ];then
				echo "The current system does not support php-8.4/8.5, it will be supported soon."
				echo "If you need to use php-8.4/8.5, it is recommended to use Debian-11+/Ubuntu22+ for installation."
				exit 1
			else
				echo "当前系统暂不支持php-8.4/8.5 会尽快支持"
				echo "如需使用php-8.4/8.5,建议使用Debian-11+/Ubuntu22+进行安装"
				exit 1
			fi
		fi
	fi
...
System_Lib(){
	if [ "${PM}" == "yum" ] || [ "${PM}" == "dnf" ] ; then
		...
		yum install perl perl-FindBin -y
...
Install_Configure(){
	...
	if [ "${php_version}" == "71" ] || [ "${php_version}" == "72" ] || [ "${php_version}" == "73" ];then
        ...
        if [ "${icu_v}" -gt "70" ] && [ -z "${USE_GCC_13}" ];then
	...
	if [ "${USE_GCC_13}" ] && [ "${php_version}" -ge "71" ];then
    ...
    if [ "${Centos7Check}" ];then
        if [ "${php_version}" == "81" ] ||[ "${php_version}" == "82" ] || [ "${php_version}" == "83" ] || [ "${php_version}" == "84" ] || [ "${php_version}" == "85" ]; then
	...
	if [ "${php_version}" == "53" ]; then
        ...
        if [ "${version}" == "8.3" ] || [ "${version}" == "8.4" ] || [ "${version}" == "8.5" ];then
			ENABLE_MBSTRING_CONFIG=""
			binutils_minor_ver=$(ld --version | head -n1 | grep -oE '[0-9]+\.[0-9]+' | head -n1|cut -f 2 -d '.')
			if [ "${binutils_minor_ver}" -lt "31" ];then
				ENABLE_MBSTRING_CONFIG="--enable-mbstring=shared"
				MBSTRING_SHARED="True"
			else
				ENABLE_MBSTRING_CONFIG="--enable-mbstring"
			fi
		    ./configure --prefix=${php_setup_path} --with-config-file-path=${php_setup_path}/etc --enable-fpm --with-fpm-user=www --with-fpm-group=www --enable-mysqlnd --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --with-iconv-dir --with-freetype --with-jpeg --with-zlib --with-libxml-dir=/usr --enable-xml --disable-rpath --enable-bcmath --enable-shmop --enable-sysvsem --enable-inline-optimization --with-curl --enable-mbregex ${ENABLE_MBSTRING_CONFIG} --enable-intl --enable-pcntl --enable-ftp --enable-gd --with-openssl --with-mhash --enable-pcntl --enable-sockets --with-xmlrpc  --enable-soap --with-gettext --disable-fileinfo --enable-opcache --with-sodium=/usr/local/libsodium --with-webp ${i_make_args} 2>&1|tee /tmp/php_config.pl
		else
...
Install_Zip_ext(){
	...
	if [ "${php_version}" == "73" ];then
		...
		extFile="/www/server/php/85/lib/php/extensions/no-debug-non-zts-20250925/zip.so"
	fi
	...
	if [ ${MBSTRING_SHARED} ];then
		extFile=$(echo "${extFile/zip.so/mbstring.so}")
		if [] -f "${extFile}" ];then
			echo "extension = mbstring.so" >> ${php_setup_path}/etc/php.ini
		fi
	fi
}
...
Download_Conf(){
    for phpV in 52 53 54 55 56 70 71 72 73 74 80 81 82 83 84 85; do
...
Uninstall_PHP()
{
	...
	for phpV in 52 53 54 55 56 70 71 72 73 74 80 81 82 83 84 85;
	do
		if [ -f "/www/server/php/${phpV}/bin/php" ]; then
			rm -f /usr/bin/php
