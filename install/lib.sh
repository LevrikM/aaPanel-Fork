...
Install_Yumlib(){
	...
	NFS6_CHECK=$(rpm -q kernel|grep nfs6)
	if [ "${NFS6_CHECK}" ];then
	...
	ALI4_OS=$(cat /etc/redhat-release |grep "Alibaba Cloud Linux release 4")
	if [  "${ALI4_OS}" ];then
...
Install_Aptlib(){
	...
	UBUNUT_VER=$(cat /etc/issue|grep -i ubuntu|awk '{print $2}'|cut -f 1 -d '.')
	if [ "${UBUNUT_VER}" ];then
		if [ "${UBUNUT_VER}" -ge "26" ];then
